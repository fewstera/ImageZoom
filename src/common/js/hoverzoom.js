...
var hoverZoom = {
    ...
    loadHoverZoom:function () {
        ...
        function hideHoverZoomImg(now) {
            ...
            //sendMessageToBackground({action: 'viewWindow', visible: false});
        }
        ...
        function displayFullSizeImage() {
            ...
            if (options.addToHistory) {
                ...
                sendMessageToBackground({action:'addUrlToHistory', url:url});
            }
            sendMessageToBackground({action:'trackEvent', event:{category:'Actions', action:'ImageDisplayedOnSite', label:document.location.host}});
            sendMessageToBackground({action:'trackEvent', event:{category:'Actions', action:'ImageDisplayedFromSite', label:imgDetails.host}});
        }
        ...
        function imgFullSizeOnError() {
            if (imgDetails.url == $(this).attr('src')) {
                ...
                if (hz.currentLink && hoverZoomSrcIndex < hz.currentLink.data().hoverZoomSrc.length - 1) {
                    ...
                    sendMessageToBackground({action:'trackEvent', event:{category:'Errors', action:'LoadingErrorFromSite', label:imgDetails.host}});
                }
            }
        }
        ...
        function imgLinksPrepared(links) {
            ...
            if (options.pageActionEnabled && !pageActionShown && showPageAction) {
                sendMessageToBackground({action:'showPageAction'});
        ...
        function loadOptions() {
            sendMessageToBackground({action:'getOptions'}, function (result) {
                options = result;
                applyOptions();
            });
        }
        ...
        function openImageInWindow() {
            sendMessageToBackground({action:'getItem', id:'popupBorder'}, function (data) {
                var createData,
        ...
        kango.addMessageListener('optionsChanged', onRequest);
        loadOptions();
    },
    ...
    createImgLoading:function () {
        hoverZoom.imgLoading = hoverZoom.imgLoading || $('<img src="http://s7.postimage.org/6x3yygf87/loading.gif" style="opacity: 0.8; padding: 0; margin: 0" />');
    ...
    preloadImages:function () {
        ...
        function preloadNextImage() {
            ...
            if (link.data().hoverZoomPreloaded) {
                preloadNextImage();
                sendMessageToBackground({action:'preloadProgress', value:preloadIndex, max:links.length});
            } else {
...
kango.console.log('Starting HoverZoom');
